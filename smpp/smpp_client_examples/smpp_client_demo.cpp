...
void asyncClientDemo2() {
  ...
  // read PDU responses
  for (int nSequenceNumber = 2; nSequenceNumber < 50; nSequenceNumber++) {
    auto pduResponse = smppClient.readPduResponse();

    if (auto submitSmResp = pduResponse->asSubmitSmResp()) {
      std::cout << "Received submit sm response with status " << submitSmResp->getCommandStatus()
                << " and seq number: " << submitSmResp->getSequenceNumber() << std::endl;
    } else {
      std::cout << "Received " << smpp::util::commandIdToString(pduResponse->getCommandId())
                << std::endl;
    }
  }
